...
for i in ./*/; do
    ...
    for region in $regions; do
        if [ "$first_char" == "${region:0:1}" ]; then
            ...
            cd "$i" 
            if [ "$1" = "save" ];  then
                cutflow_compare --files histoOut-*.root -r $region --colored --counts PAPER_CUTFLOW --relative-error --save --labels ibeuler_selection
            else 
                cutflow_compare --files histoOut-*.root -r $region --colored --counts PAPER_CUTFLOW --relative-error --labels ibeuler_selection
            fi
            cd ..
        fi
    done
